...
links or dashboards. It must expose a buyer-executable close room, checkout path,
...
CLOSE_ROOM = ROOT / "website" / "auto-parts-buyer-close-room.html"
...
def main() -> None:
    ...
    require(CLOSE_ROOM.exists(), "missing APF buyer close room page")
    ...
    close_room = CLOSE_ROOM.read_text(encoding="utf-8")
    ...
    required_root_markers = [
        ...
        "buyer-ready APF instant checkout, buyer close room",
        "./website/auto-parts-buyer-close-room.html?product=auto-parts-price-finder&priceEur=29&buyer=used-parts-seller&source=root-launcher",
        "Uždaryti pirkėją 29 € close room",
    ...
    require("const CLOSE_LEDGER_KEY='apfBuyerCloseRoomLedger'" in close_room, "close room must keep close-room ledger")
    require("function buildBuyerMailto(row)" in close_room, "close room must create buyer email handoff")
    require("function buildProofMailto(row)" in close_room, "close room must create paid-proof handoff")
    require("revenueCountedEur:0" in close_room, "close room must keep zero-revenue guard")
    ...
    print(
        "checked: public root exposes APF 29 EUR buyer close room, buyer checkout, payment destination setup, "
        "APF paid confirmation handoff, zero-revenue guard, and rejects root/page/close/checkout clicks as revenue"
    )
